...
def obtener_totales(caja):
    return obtener_totales_caja(caja)
...
def inicio(request):
    ...
    saldo_efectivo_esperado = Decimal('0.00')
    saldo_real_total = Decimal('0.00')
    estado_cierre_actual = ''
    ...
    if caja_hoy:
        ...
        saldo_efectivo_esperado = totales['saldo_efectivo_esperado']
        saldo_digital_esperado = totales['saldo_digital_esperado']
        if cierre_hoy:
            saldo_real_total = (
                cierre_hoy.saldo_real + cierre_hoy.saldo_real_digital
            )
            diferencia_actual = calcular_diferencia_cierre(
                caja_hoy,
                cierre_hoy,
                totales,
            )
            estado_cierre_actual = obtener_estado_cierre(diferencia_actual)
    ...
    if request.method == 'POST':
        ...
        if accion == 'editar':
            return editar_cierre(request, caja_hoy, cierre_hoy, totales)
        ...
        saldo_real_total = saldo_real + saldo_real_digital
        diferencia = saldo_real_total - saldo_esperado
    ...
    contexto = {
        ...
        'estado_cierre_actual': estado_cierre_actual,
...
def editar_cierre(request, caja_hoy, cierre_hoy, totales):
    ...
    cierre_hoy.total_ingresos = totales['total_ingresos']
    cierre_hoy.total_egresos = totales['total_egresos']
    cierre_hoy.saldo_esperado = saldo_esperado
